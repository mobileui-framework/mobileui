	...
	var methods = {
			...
			return this.each(function() {
				...
				if ($.type($this.data('wait-attributes'))=="undefined") {
					...
					var attributes = $.extend({
				   		...
				   		bgOpacity: "0.3",
				   		bgAlphaFilter: "80",
				   		bgImage: "/vendors/jqwait/wait.gif",
						id: "wait-id-"			
					}, options);
		...
		show : function() {
			return this.each(function() {
				...
				$wait = $('#'+dt.id);
				if ($wait==null || ($wait.length!=null && $wait.length==0)) {
					$div = $("<div id='" + dt.id + "'/>");
					$div.css({'display':'block','position': dt.position, 'z-index':dt.zIndex,'top':'0','left':'0','height':'100%','width':'100%'});
					$div.css({"background-color":dt.bgColor, 'background-image':"url('" + dt.bgImage + "')",'background-position':'50% 50%','background-repeat':'no-repeat','background-size':'31px 31px'});
